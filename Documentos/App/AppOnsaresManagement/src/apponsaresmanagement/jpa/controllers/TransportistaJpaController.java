...
public class TransportistaJpaController implements Serializable {
    ...
    public List<Transportista> findDriverContainsName(String pieceOfName) {
        EntityManager em = getEntityManager();
        pieceOfName = "%" + pieceOfName + "%";
        return em.createNamedQuery("Transportista.containsName").setParameter("nombre", pieceOfName).getResultList();
    }

    public Transportista findDriverbyNameAndSurname(String nameSurname) {
        EntityManager em = getEntityManager();
        nameSurname = nameSurname.trim();
        String name, surname1, surname2;
        String[] driverData = nameSurname.split(" ");
        name = driverData[0].trim();
        surname1 = driverData[1].trim();
        surname2 = driverData[2].trim();
        return (Transportista) em.createNamedQuery("Transportista.findByNameAndSurname")
                .setParameter("nombre", name)
                .setParameter("apellido1", surname1)
                .setParameter("apellido2", surname2).getResultList().get(0);
